...
15 domains assessed. **1 (7%) are spoofable**, meaning forged mail claiming that domain is delivered to the inbox. A further 5 are only partially protected.
...
| berkshirehathaway.com | WEAK | `p=reject` | DMARC p=reject, but the SPF record could not be retrieved, so the SPF half is unverified |
...
| mckesson.com | PROTECTED | `p=reject` | DMARC p=reject at 100% with a valid SPF record |
| microsoft.com | PROTECTED | `p=reject` | DMARC p=reject at 100% with a valid SPF record |
